...
from mmrotate.core import eval_rbbox_map, eval_rbbox_metric, obb2poly_np, poly2obb_np
...
class DOTADataset(CustomDataset):
    ...
    # for fair1m2.0
    CLASSES = ('PassengerShip', 'Motorboat', 'FishingBoat', 'Tugboat', 'other-ship',
               'EngineeringShip', 'LiquidCargoShip', 'DryCargoShip', 'Warship',
               'SmallCar', 'Bus', 'CargoTruck', 'DumpTruck', 'other-vehicle',
               'Van', 'Trailer', 'Tractor', 'Excavator', 'TruckTractor',
               'Boeing737', 'Boeing747', 'Boeing777', 'Boeing787', 'ARJ21', 'C919',
               'A220', 'A321', 'A330', 'A350', 'other-airplane',
               'BaseballField', 'BasketballCourt', 'FootballField', 'TennisCourt', 'BasketballCourt',
               'Roundabout', 'Intersection', 'Bridge', 'Intersection')
    """       
        
    # for super classes of fair1m2.0
    # CLASSES = ('airplane', 'ship', 'vehicle')
    CLASSES = ('ship', 'vehicle', 'airplane')
    ...
    def evaluate(self,
        ...
        if metric == 'mAP':
            ...
            ship_ap, vehicle_ap, airplane_ap, _ = eval_rbbox_metric(results,
                annotations,
                scale_ranges=scale_ranges,
                iou_thr=iou_thr,
                dataset=self.CLASSES,
                logger=logger,
                nproc=nproc)  
            eval_results['Mean Recall'] = mean_recall            
            eval_results['Mean Precision'] = mean_precision 
            eval_results['Mean F1 Score'] = mean_f1 
            eval_results['mAP'] = mean_ap 
            eval_results['Ship Recall'] = ship_recall          
            eval_results['Vehicle Recall'] = vehicle_recall 
            eval_results['Airplane Recall'] = airplane_recall 
            eval_results['Ship Precision'] = ship_precision 
            eval_results['Vehicle Precision'] = vehicle_precision 
            eval_results['Airplane Precision'] = airplane_precision   
            eval_results['Ship F1 Score'] = ship_f1 
            eval_results['Vehicle F1 Score'] = vehicle_f1 
            eval_results['Airplane F1 Score'] = airplane_f1
            eval_results['Ship AP'] = ship_ap 
            eval_results['Vehicle AP'] = vehicle_ap      
            eval_results['Airplane AP'] = airplane_ap 
        else:
